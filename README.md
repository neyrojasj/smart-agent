...
```bash
pip install smartagent
cd your-project
smart init
```

**Install directly from GitHub** (latest unreleased):

```bash
pip install git+https://github.com/neyrojasj/smart-agent.git
cd your-project
smart init
```
...
| Option | Command |
|--------|--------|
| **Full Install** (with standards) | `smart init` |
| **No Standards** | `smart init --no-standards` |
| **Minimal** | `smart init --minimal` |
| **Force overwrite** | `smart init --force` |
| **Shell fallback** (no Python) | `curl -sSL https://raw.githubusercontent.com/neyrojasj/smart-agent/main/scripts/install.sh \| bash` |
...
├── pyproject.toml               # 📦 Python package (pip install smartagent)
├── smartagent/                  # 🐍 CLI source
│   ├── __init__.py
│   ├── cli.py                   # Click entry point: init, sync, update
│   ├── installer.py             # Download + install into project
│   └── sync.py                  # Personal branch git operations
├── .github/
...
1. **Bump the version** in `smartagent/__init__.py` and `pyproject.toml`
2. **Build the distribution**:
   ```bash
   python3 -m build
   ```
   This creates `dist/smartagent-X.Y.Z.tar.gz` and `dist/smartagent-X.Y.Z-py3-none-any.whl`.
3. **Test on TestPyPI first** (optional but recommended):
   ```bash
   twine upload --repository testpypi dist/*
   pip install --index-url https://test.pypi.org/simple/ smartagent
   ```
4. **Publish to PyPI**:
   ```bash
   twine upload dist/*
...
> **Name check**: Verify `smartagent` is available on [pypi.org/project/smartagent](https://pypi.org/project/smartagent) before the first upload.
...
[![Star on GitHub](https://img.shields.io/github/stars/neyrojasj/smart-agent?style=social)](https://github.com/neyrojasj/smart-agent)
